...
public class AllJoysticks implements AutoCloseable {
    ...
    private final Map<Path, Future<Boolean>> activeJoysticks = new ConcurrentHashMap<>();
     ...
     */
    public synchronized Map<Path, Future<Boolean>> getActiveJoysticks() {
        Set<Path> toRemove = new HashSet<>();
        activeJoysticks.forEach((path, future) -> {
            if (future.isDone()) {
                LOGGER.info("Removing joystick " + path + " from active pool.");
                toRemove.add(path);
            }
        });
        ...
        try {
            activeJoysticks.put(
                    ...
                    executorService.submit(() -> {
                        try {
                            LOGGER.info("Joystick stream thread spawned for " + path);
                            addOns.apply(JSTestReader
                                    .ofJoystick(path)
                                    .takeWhile(l -> open)).forEach(sink);
                            LOGGER.info("Closed Stream for Joystick at " + path);
                            return true;
                        } catch (Exception e) {
                            LOGGER.log(Level.INFO, "Exception streaming joystick " + path, e);
                            return false;
                        }
                    }));
            LOGGER.info("");
            ...
            LOGGER.info("Joystick " + path.toString() + " exception joining pool");
